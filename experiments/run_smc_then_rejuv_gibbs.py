"""Run SMC for T steps, then rejuvenation-only for T steps, and save Gibbs steps results."""
...
def run_smc_then_rejuv_gibbs(seed, T, B, N_train, N_test, D, K, C, P, α_pi, α_theta, X_test, true_π, true_θ):
    """Run SMC then rejuvenation-only and track Gibbs steps."""
    print(f"Running SMC then rejuvenation-only (seed={seed})...")
    ...
    N, D, K = warmup_X.shape
    keys = jax.random.split(warmup_key, P + 1)
    init_keys = keys[1:]
    ...
    try:
        # Warmup SMC step
        I_B = jnp.arange(0, min(B, 50))
        X_B = warmup_X[I_B]
        particles = (A, φ, π, θ)
        _ = smc_step(warmup_key, particles, log_weights, log_gammas, X_B, I_B, α_pi, α_theta)
        
        # Warmup Gibbs
        I_rejuv = jnp.arange(0, min(B, 50))
        X_rejuv = warmup_X[I_rejuv]
        _ = gibbs(warmup_key, X_rejuv, I_rejuv, A[0], φ[0], π[0], θ[0], α_pi, α_theta)
        
        # Warmup test likelihood
        particles_temp = (A, φ, π, θ)
        _ = compute_weighted_test_loglik(X_test[:10], particles_temp, log_weights)
    except:
        pass
    ...
    for t in tqdm(range(T)):
        ...
        # Record results - 1 SMC step = 1 Gibbs step
        current_gibbs_steps = t + 1
        logliks.append(float(loglik_t) / N_test)
        gibbs_steps.append(current_gibbs_steps)
    
    # Phase 2: Rejuvenation-only steps
    print("  Phase 2: Rejuvenation-only steps...")
    for t in tqdm(range(T)):
        ...
        # Compute test log-likelihood (weights remain unchanged)
        particles_test = (A, φ, π, θ)
        loglik_t = compute_weighted_test_loglik(X_test, particles_test, log_weights)
        
        # Record results - 1 rejuvenation step = 1 Gibbs step
        # Continue counting from the end of SMC phase
        current_gibbs_steps = T + (t + 1)
        logliks.append(float(loglik_t) / N_test)
        gibbs_steps.append(current_gibbs_steps)
...
def main():
    ...
    print("SMC Then Rejuvenation-Only - Gibbs Steps Experiment")
    print("=" * 60)
    print(f"Configuration: P={P}, T={T} SMC + T={T} Rejuv, B={B}, Runs={n_runs} (first discarded)")
    ...
    for i, seed in enumerate(seeds):
        ...
        logliks, gibbs_steps = run_smc_then_rejuv_gibbs(
