"""Run SMC without rejuvenation and save Gibbs steps results."""

import jax
import jax.numpy as jnp
from tabsmc.smc import smc_no_rejuvenation, init_empty
import sys
import os
...
def run_smc_no_rejuvenation_gibbs(seed, T, B, N_train, N_test, D, K, C, P, α_pi, α_theta, X_test, true_π, true_θ):
    """Run SMC without rejuvenation and track Gibbs steps."""
    print(f"Running SMC without rejuvenation (seed={seed})...")
    ...
    try:
        _ = smc_no_rejuvenation(warmup_key, warmup_X, 2, P, C, min(B, 50), α_pi, α_theta)
    ...
    for t in tqdm(range(T)):
        ...
        # SMC step (import smc_step directly)
        from tabsmc.smc import smc_step
        key, subkey = jax.random.split(key)
        particles = (A, φ, π, θ)
...
def main():
    ...
    print("SMC Without Rejuvenation - Gibbs Steps Experiment")
    print("=" * 60)
    ...
    for i, seed in enumerate(seeds):
        ...
        logliks, gibbs_steps = run_smc_no_rejuvenation_gibbs(
        ...
        if i == 0:
            ...
            # Discard timestep 0 and adjust gibbs_steps to start from 1
