...
# Sampling functions
categorical = MappedFunction(jax.random.categorical)
# write a function to sample from a log dirichlet
def sample_log_dirichlet(key, α):
    x = jax.random.loggamma(key, α)
    return x - jax.scipy.special.logsumexp(x)

log_dirichlet = MappedFunction(sample_log_dirichlet)

# Scoring functions
def score_log_dirichlet(α, π):
    return jnp.sum((α - 1.0) * π) - jax.scipy.special.gammaln(α).sum() + jax.scipy.special.gammaln(jnp.sum(α))

score_log_dirichlet = MappedFunction(score_log_dirichlet)
